	...
	public void add(PlaceRecord listItem) {
		...
		if (storeBitmapToFile(listItem.getFlagBitmap(), filePath)) {
			...
			batchOperation.add(ContentProviderOperation
					.newInsert(PlaceBadgesContract.CONTENT_URI)
					.withValue(PlaceBadgesContract.COUNTRY_NAME,
							listItem.getCountryName())
					.withValue(PlaceBadgesContract.PLACE_NAME,
							listItem.getPlace())
					.withValue(PlaceBadgesContract.LAT, listItem.getLat())
					.withValue(PlaceBadgesContract.LON, listItem.getLon())
					.withValue(PlaceBadgesContract.FLAG_BITMAP_PATH,
							listItem.getFlagBitmapPath()).build());
